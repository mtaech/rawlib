...
use crate::cli::{Config, OverwritePolicy, ProcessMode, ProcessStats, Verbosity};
...
impl BatchProcessor {
    ...
    pub fn process(&self) -> Result<ProcessStats> {
        ...
        info!(
            "Batch processing completed: {} success, {} failed, {} skipped",
    ...
    fn determine_output_path(&self, input_file: &Path) -> PathBuf {
        match self.config.mode {
            ...
            ProcessMode::Batch => {
                ...
                self.config.output.join(file_stem).with_extension(extension)
            }
        }
    }
